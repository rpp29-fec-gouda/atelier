...
class Outfit extends react_1.default.Component {
    constructor(props) {
        super(props);
    ...
    render() {
        ...
        let key = 0;
        return selectedProduct ? (<div id='Outfit' onClick={handleTracking}>
        <h1></h1>
        ...
        <div className='rp-card-container'>
          ...
          {Array.isArray(outfit) && outfit.length ? (outfit.map(id => (<ProductCard_jsx_1.default key={`outfitCard${id}`} type='Outfit' productId={id} selectProduct={selectProduct} action={removeFromOutfit} checkCache={checkCache} updateCache={updateCache} clickTracker={clickTracker}/>))) : null}
        </div>
      </div>) : null;
    }
}
