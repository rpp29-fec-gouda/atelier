...
const RatingList_jsx_1 = __importDefault(require("./RatingList.jsx"));
const ReviewsList_jsx_1 = __importDefault(require("./ReviewsList.jsx"));
const NewReview_jsx_1 = __importDefault(require("./NewReview.jsx"));
const axios_1 = __importDefault(require("axios"));
require("./ratingsAndReviews.css");
class RatingsAndReviews extends react_1.default.Component {
    constructor(props) {
        super(props);
        this.reviews = [];
        this.reviewsData = {};
    ...
    getReviews(sort, count, id, callback = () => { }) {
        ...
        if (reviews) {
            ...
            axios_1.default.get(`reviews?sort=${sort}&count=100&product_id=${id}`)
                .then(res => {
    ...
    getRatings(id, callback = () => { }) {
        ...
        if (ratings) {
            ...
            axios_1.default.get(`reviews/meta?product_id=${id}`)
                .then(res => {
    ...
    render() {
        ...
        return (<div id='rr-ratings-reviews-widget' id='rr-ratings-reviews-widget' onClick={this.handleInteractions}>
        ...
        <div id='rr-ratings-reviews'>
          <RatingList_jsx_1.default ratings={this.state.ratings} ratingDetails={this.ratingDetails} averageRating={this.averageRating} totalRating={this.totalRating} reviews={this.state.reviews} characteristics={this.state.characteristics} recommended={this.state.recommended} handleRatingProgressFilter={this.handleRatingProgressFilter}/>

          {this.state.reviews ?
                <ReviewsList_jsx_1.default selectedProduct={this.props.selectedProduct} averageRating={this.averageRating} roundedAverage={this.roundedAverage} characteristics={this.state.characteristics} loadMoreReviews={this.loadMoreReviews} reviews={this.state.reviews} displayedReviews={displayedReviews} sortOptions={this.sortOptions} handleReviewSort={this.handleReviewSort} getReviews={this.getReviews} getRatings={this.getRatings} currentSort={this.state.sort} product_id={this.state.product_id} count={this.state.count} reviewsLength={this.state.reviewsLength} callback={(filteredList) => this.updateReviewsList(filteredList)} expanded={this.state.expanded} expandBody={this.expandBody}/>
                : <NewReview_jsx_1.default selectedProduct={selectedProduct} characteristics={this.characteristics}/>}

        </div>
      </div>);
    }
}
exports.default = RatingsAndReviews;
